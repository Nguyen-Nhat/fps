...
// The ProcessingFileRowGroupFunc type is an adapter to allow the use of ordinary
// function as ProcessingFileRowGroup mutator.
type ProcessingFileRowGroupFunc func(context.Context, *ent.ProcessingFileRowGroupMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ProcessingFileRowGroupFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ProcessingFileRowGroupMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProcessingFileRowGroupMutation", m)
	}
	return f(ctx, mv)
}
