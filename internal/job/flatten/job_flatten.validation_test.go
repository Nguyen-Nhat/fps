...
func Test_validateAndGetValueForRequestFieldExcel(t *testing.T) {
	...
	tests := []struct {
		...
		{"test case wrong column key -> error", args{1, []string{}, &reqFieldKeyWrong}, "", true},
		{"test case field require but key out of column range -> error", args{1, []string{"val1", "val2"}, &reqFieldKeyABRequired}, "", true},
		{"test case field require but value is empty -> error", args{1, []string{"", "val2"}, &reqFieldKeyARequired}, "", true},

		{"test case field A require and key in column range -> correct value", args{1, []string{"val1", "val2"}, &reqFieldKeyARequired}, "val1", false},
		{"test case field A NOT require, key in column range -> correct value", args{1, []string{"", "val2"}, &reqFieldKeyANotRequired}, "", false},
